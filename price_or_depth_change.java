...
public class price_or_depth_change {
	...
	public static void main(String[] args) throws IOException {
		...
		int sep = 3;
		String writedir = "\\price_or_depth_change\\daily\\";
		...
		for (String dy : datayear) {
			...
			for (int i = 0; i < filelist.length; i++) {
				...
				File file = new File(currentdir + dy + writedir + fileID + "_.csv");
				...
				while ((line = brtxt.readLine()) != null) {
					...
					if (time_second >= 90000 && time_second <= 151500) {
						...
						if (record2.equals(" 0")) {
							...
							if (!asktemp.equals(price) || !askdepth.equals(volume)) {
								write = true;
							}
							asktemp = price;
							askdepth = volume;
						} else if (kind.equals("128")) {
							if (!bidtemp.equals(price) || !biddepth.equals(volume) || write) {
								wline = fileID + "," + time.substring(0, 2) + ":" + time.substring(2, 4) + ":" + second
