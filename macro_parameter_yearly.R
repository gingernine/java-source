...
rootdir <- "C:\\Users\\kklab\\Desktop\\yurispace\\board_fluctuation\\src\\nikkei_needs_output"
subdir <- "\\statistics_of_the_limit_order_book\\yearly"
datayear <- "\\2007"

filepath <- paste(rootdir, subdir, datayear, "_.csv", sep="", collapse=NULL)
...
wfilename <- paste(rootdir, subdir, datayear, "_statistics_summary", ".csv", sep = "", collapse = NULL)
...
for (r in seq(nrow(summary))) {
    ...
    for (c in seq(ncol(summary))) {
        if (c == ncol(summary)) {
            line <- paste(line, "$", round(summary[r, c],3), "$", " \\\\ \\hline", sep="", collapse=NULL)
        } else {
            line <- paste(line, "$", round(summary[r, c],3), "$", " & ", sep="", collapse=NULL)
        }
    }
    code <- rbind(code, matrix(line, nrow=1, ncol=1))
}

wfilename <- paste(rootdir, subdir, datayear, "_texcode", ".csv", sep = "", collapse = NULL)
