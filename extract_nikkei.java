...
//(nikkei needs)日経平均先物の約定データ，最良気配値のデータ(限月調整済み)を抽出するプログラム
public class extract_nikkei {

	public static void main(String[] args) throws IOException {

		String currentdir = "C:\\Users\\kklab\\Desktop\\yurispace\\board_fluctuation\\src\\nikkei_needs_output";
		String datayear[] = { "\\2015", "\\2016" };
		String dirmonth[] = { "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12" };
		...
		for (String dy : datayear) {
			for (String mth : dirmonth) {

				String datapath = "E:\\日経平均先物" + dy + mth;
				...
				for (int i = 0; i < filelist.length; i++) {
					...
					if (rfilename.length == 4) {
						// zipファイルは飛ばす．
						if (!rfilename[3].equals("zip")) {
							System.out.println("error");
						}
						continue;
					}

					if (rfilename.length == 3) {
						fileID = rfilename[2];
					} else {
						fileID = "1";
					}

					File file = new File(currentdir + dy + "\\raw\\" + rfilename[1] + "_" + fileID + "_.txt");
					...
					while ((line = brtxt.readLine()) != null) { // データを一行ずつロードする
						...
						if (date < 20100720) {
							// 2010年7月20日を境にレコード種別のデータの表示位置が変わる．
