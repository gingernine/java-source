...
//約定データは約定種別も記録する．同時点の最良気配は売りと買いを一行に並べる．
public class raw_to_csv_2{
            	...
            	//9:00:00時点の最良買い気配値と最良売り気配値を取得する．
             	...
             	if(time_second >= 90000 && time_second <= 151500){
             		...
             		if(record2.equals(" 0")){
             			...
             			pw.println(wline);
             			bidtemp = price; //価格の変化を記録する．
             			biddepth = volume; //数量の変化を記録する．
             		} else if (kind.equals("  0")) {
             			wline = fileID + "," + time.substring(0,2) + ":" +
             					time.substring(2,4) + ":" + second + ",Quote,,," +
             					bidtemp + "," + biddepth + "," + price + "," + volume + ",,";
             			pw.println(wline);
             			asktemp = price; //価格の変化を記録する．
             			askdepth = volume; //数量の変化を記録する．
             		}
             	}
            }
            brtxt.close();
