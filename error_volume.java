...
public class error_volume {
	...
	public static void main(String[] args) throws IOException {
		...
		// String datadir = "\\bid_more_than_ask_from_rawcsv_2";
		// String datadir = "\\bid_equal_to_ask_from_rawcsv_2";
		String datadir = "\\out_of_range_from_rawcsv_2";
		// int sep = 9; // [bid_more_than_ask_from_rawcsv_2, bid_equal_to_ask_from_rawcsv_2]
		int sep = 8; // [out_of_range_from_rawcsv_2]
		...
		File rfilepath = new File(currentdir + datayear + datadir + "\\daily"); // 読み込むファイルのディレクトリのパス．
		File[] filelist = rfilepath.listFiles(); // 読み込むファイル名を取得する．
		File newdir = new File(currentdir + datayear + datadir + task);
		if (!newdir.exists() && rfilepath.exists()) {
			newdir.mkdirs();
		}
		...
		if (task.equals("\\date_error")) {
			...
			for (int i = 0; i < filelist.length; i++) {
				...
				brtxt.close();
				fr.close();
				pw.println(rfiledate.substring(0, 4) + "/" + rfiledate.substring(4, 6) + "/" + rfiledate.substring(6, 8)
						+ "," + count + ",");
				count = 0;
			}
		}
