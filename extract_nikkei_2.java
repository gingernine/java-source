...
//(nikkei needs)2010年7月16日前のフォーマットでの日経平均先物の約定データ，最良気配値のデータ(限月調整済み)を抽出するプログラム
public class extract_nikkei_2{
    ...
    public static void main(String[] args) throws IOException{
    	...
    	for(String dy: datayear){
            ...
            for(int i=0; i<filelist.length; i++){
                ...
                System.out.println(rfilename[1]);

             	File file = new File(currentdir + dy + "\\raw\\" + rfilename[1] + "_" + rfilename[2] + "_.txt" );
             	PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(file)));

             	//データが取られた年を知る
             	int year = Integer.parseInt(rfilename[1].substring(0,4));
            	...
            	//期近限月のデータを取り出す．
             	...
             	int month = Integer.parseInt(rfilename[1].substring(4,6));
                ...
                while ((line = brtxt.readLine()) != null) { //txtファイル名を一行ずつロードする
                	...
                	//先物である条件を満たすデータを選別
                	if((record1.equals("3") && exchange.equals("21")
                		&& security.equals("20") && code1.equals("16")
                		&& code3.equals("0018"))){
                		//次に期近限月のデータを抽出する
                		if(code2.equals(yearcode + maturity[term]) && day < maturitydate[term+1] && day >= maturitydate[term]){
                			//最後に約定データ，最良気配値を収録する．
